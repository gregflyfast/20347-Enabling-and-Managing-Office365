...
1. Use Microsoft Edge to sign in to Exchange admin center as  **Beth@Adatumyyxxxxx.hostdomain.com**, where *yyxxxxx* is your unique Adatum number, with the password you created in Module 1.
...
2. Create a new rule that sends messages to a moderator, with the following settings:
    - Name:  **Moderate Project Managers**
    - Apply the rule if:  **The recipient is a member of Project Managers**
    - Do the following:  **Forward the message for approval to Beth Burke**
...
2. Configure undeliverable journal reports to be sent to Beth Burke.
...
3. Create a new journal rule with the following settings:
    ...
    - If the message is sent to or received from: the  **Operations** group
...
4. Review the most recent message sent from **Francisco** to **Cai**, and then verify that the message was sent for moderation.
...
2. Modify the Default malware filter to:
    ...
    - Notify  **Beth@Adatumyyxxxxx.hostdomain.com**, where *yyxxxxx* is your unique Adatum number, when messages from internal or external senders are blocked
...
2. Create a new spam filter with the following settings:
    ...
    - Applied to: mails sent to  **Sales** group
...
4. On LON-CL1, in Outlook, create a new message for  **Kendra Sexton,** and the attach the **C:\\Windows\\Logs\\DISM\\dism.log** file.
...
2. Connect to Exchange Online as Olivia Emerson, using the password you created in Module 1

3. Create a new client access rule with the following settings:
    - **Name**: Block ActiveSync
    - **Block**: Exchange ActiveSync clients
    - **Exception**: IPv4 address range 192.168.10.1/24

3. Test the rule using the following settings:
    - **User**: Beth Burke
...
4. Test the rule using the following settings:
    - **User**: Beth Burke
    - **Authentication**: Basic
    - **Protocol**: Outlook Web App
    - **IPv4 address**: 172.17.17.26
    - **Remote port**: 443

  > The command **Test-ClientAccessRule** will return the name of a client access rule when there is a match. Nothing is returned when there is no match.
...
2. Edit the Exchange ActiveSync Access settings to quarantine new mobile devices and notify  **Beth Burke**.
