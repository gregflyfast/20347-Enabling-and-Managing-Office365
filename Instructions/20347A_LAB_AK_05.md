...
27. Next to All PC and mobile apps, set the value to  **On**. Verify that Office and Skype for Business are both set to on, and select  **Save**. 

28. Select  **Close**. 

29. In  **Microsoft Edge**, on the  **User Software** page, select **Ada Russell**'s profile photo icon, and then select  **Sign out**.
...
38. Leave this page open and continue to the next lab to perform the user-driven installation.
...
9. On the  **Start** screen, select **Word**. On the first things first window select  **Accept**.
